...
def gbt_reg():
    reg = GradientBoostingRegressor(n_estimators=156,subsample=0.9,
                                    learning_rate=0.045,
                                    max_depth = 3,
...
def main():
    ...
    figpath = Path(r'G:\DL_transmitance\figures\multi_regressors\layer-by-layer-4feature-selection\ch12\cloud0.3\no outliers')
    frank = figpath/'feature_rank.xlsx'
    ...
    # vstar = var_star(X[:,nlv:nlv*2],nlv)
    # layerp = layer_profile(X[:,nlv:nlv*2],nlv)
    ...
    outlier_train = [18,37,38,46,72,95,99,109,111,117,134,145,159]
    outlier_test = [2,9,10,13,20]
...
################################################################################################################
        ...
        testy = Y[test_index]
        trainy = np.delete(trainy,outlier_train,axis=0)
        testy = np.delete(testy,outlier_test,axis=0)
        ...
        # bias_levs_train, rmse_levs_train, mre_train = calc_err(train_preds,trainy)
        # bias_levs_test, rmse_levs_test, mre_test = calc_err(test_preds,testy)
        # bias_levs_train_rttov,rmse_levs_train_rttov,mre_train_rttov = calc_err(tao_rttov[train_index,:],trainy)
        # bias_levs_test_rttov,rmse_levs_test_rttov,mre_test_rttov = calc_err(tao_rttov[test_index,:],testy)
        ...
        # fignum += 1
        # plt.figure(fignum,dpi=300)
        # scatter_line_trans(trainy,train_preds,tao_rttov[train_index,ii],title,train_mae,train_rmse,train_msle,bias_rttov_train,rmse_rttov_train,msle_rttov_train)
        # figname = method+'_train_trans_layer'+str(ii+1)+'.png'
        # plt.savefig(figpath/figname)
        # # plt.close()
        # fignum += 1
        # plt.figure(fignum,dpi=300)
        # scatter_line_trans(testy,test_preds,tao_rttov[test_index,ii],title,test_mae,test_rmse,test_msle,bias_rttov_test,rmse_rttov_test,msle_rttov_test)
        # figname = method+'_test_trans_layer'+str(ii+1)+'.png'
        # plt.savefig(figpath/figname)
        # plt.close()

        # fignum += 1
        # plt.figure(fignum,dpi=300)
        # plot_trans_level(np.mean(testy,axis=0), np.mean(test_preds,axis=0), np.mean(tao_rttov[test_index,:],axis=0), p, title)
        # figname = method+'_test_trans_level.png'
        # plt.savefig(figpath/figname)
        ...
        plt.figure(fignum,dpi=300)
        plot_error_level(bias_levs_test, rmse_levs_test,bias_levs_test_rttov,rmse_levs_test_rttov,p,title)
        figname = method+'_test_bias_nonorm_log.png'
        plt.savefig(figpath/figname)
        # plt.close()
        fignum += 1
        plt.figure(fignum,dpi=300)
