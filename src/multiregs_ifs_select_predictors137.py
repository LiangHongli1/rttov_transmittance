...
from plot_map_funcs import plot_bt,plot_bias_bt,plot_error_level,plot_mre,scatter_line_trans,plot_trans_level,plot_single_var
from calc_BT import bt_GBT
from LUT import ch_info,feature_names137,trans_limit
...
def gbt_reg():
    reg = GradientBoostingRegressor(n_estimators=290,subsample=0.7,
                                    learning_rate=0.045,
                                    max_depth = 4,
                                    n_iter_no_change=500,
                                    random_state=2019,loss='ls',
...
def main():
    ...
    figpath1 = Path(r'G:\DL_transmitance\figures\multi_regressors\check')
    pdata = pd.read_excel(pfile,sheet_name='137L')
    ...
    with h5py.File(femis,'r') as f:
        emiss = f['emissivity'][:,11]
        # taosrf = f['transmission_RTTOV'][:,11,-1]
        # bt_true = f['BT_true'][:,11]
        bt_rttov = f['BT_RTTOV'][:,11]
        X = f['X'][:,:-1]
        tao_rttov = f['transmission_RTTOV'][:,11,:]
    ...
    X = np.delete(X,[548,549,550,551,554,555],axis=1) # kick out 2m variables
    npro = X.shape[0]
...
################################# only ocean ##############################3
    ...
    seeds = random.sample(range(2020),10)
    for s in seeds:
        X_train,X_test,Y_train,Y_test = train_test_split(X,Y,test_size=0.2,random_state=s)
        _,_,tsrf_train,tsrf_test = train_test_split(X,X[:,548],test_size=0.2,random_state=s)
        _,_,emis_train,emis_test = train_test_split(X,emiss,test_size=0.2,random_state=s)
        tao_rttov_train,tao_rttov_test,bt_rttov_train,bt_rttov_test = train_test_split(tao_rttov,bt_rttov,test_size=0.2,random_state=s)
        _,_,bt_true_train,bt_true_test = train_test_split(X,bt_true,test_size=0.2,random_state=s)
    ...
    ###################################################################################
        for j,ch in enumerate(chs):
            ...
            ############################################################################################
            tlevel_train = X_train[:,nlv:nlv*2]
            taosrf_train = trans_preds_train[:,-1]

            bt_preds_train,btair_preds_train = bt_GBT(trans_preds_train[:,:-1],tlevel_train,emis_train,tsrf_train,taosrf_train,
...
############################################## Plot good and bad performance profile ###########
            ...
            fignum += 1
            plt.figure(dpi=300)
            plot_single_var(X_train[bias_rank[0],nlv:nlv*2],p,'temperature','training good profile')
            figname = 'train_t_good.png'
            plt.savefig(figpath/figname)
            fignum += 1
            plt.figure(dpi=300)
            plot_single_var(X_train[bias_rank[-1],nlv:nlv*2],p,'temperature','traning bad profile')
            figname = 'train_t_bad.png'
            plt.savefig(figpath/figname)
            fignum += 1
            plt.figure(dpi=300)
            plot_single_var(X_train[bias_rank[0],nlv*2:nlv*3],p,'water vapor','traning good profile')
            figname = 'train_q_good.png'
            plt.savefig(figpath/figname)
            fignum += 1
            plt.figure(dpi=300)
            plot_single_var(X_train[bias_rank[-1],nlv*2:nlv*3],p,'water vapor','traning bad profile')
            figname = 'train_q_bad.png'
            plt.savefig(figpath/figname)

            ################################# test BT########################
            tlevel_test = X_test[:,nlv:nlv*2]
            taosrf_test = trans_preds_test[:,-1]
            bt_preds_test,btair_preds_test = bt_GBT(trans_preds_test[:,:-1],tlevel_test,emis_test,tsrf_test,taosrf_test,
                                                    ch_info[channel]['scale'],ch_info[channel]['offset'],ch_info[channel]['cv'])
...
###############################################################################################################
            ...
            plt.figure(dpi=300)
            plot_single_var(X_train[bias_rank[0],nlv:nlv*2],p,'temperature','training good profile')
            figname = 'test_t_good.png'
            plt.savefig(figpath/figname)
            fignum += 1
            plt.figure(dpi=300)
            plot_single_var(X_train[bias_rank[-1],nlv:nlv*2],p,'temperature','traning bad profile')
            figname = 'test_t_bad.png'
            plt.savefig(figpath/figname)
            fignum += 1
            plt.figure(dpi=300)
            plot_single_var(X_train[bias_rank[0],nlv*2:nlv*3],p,'water vapor','traning good profile')
            figname = 'test_q_good.png'
            plt.savefig(figpath/figname)
            fignum += 1
            plt.figure(dpi=300)
            plot_single_var(X_train[bias_rank[-1],nlv*2:nlv*3],p,'water vapor','traning bad profile')
            figname = 'test_q_bad.png'
            plt.savefig(figpath/figname)
##########################################################################################
