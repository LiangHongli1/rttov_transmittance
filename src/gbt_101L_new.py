...
def gbt_reg(loss,ntree,ndepth):
    reg = GradientBoostingRegressor(n_estimators=ntree,subsample=0.8,
                                    learning_rate=0.045,
                                    max_depth = ndepth,
...
def main():
    ...
    # figpath1 = Path(r'G:\DL_transmitance\figures\101L\final\reg_result\depth5\200tree')
    modelpath = Path(r'G:\DL_transmitance\models\GBT')
    ...
    #     model = rf_reg()
    # elif method=='GBT':
    #     model = gbt_reg(loss,ntree,ndepth)
    # elif method=='XGBoost':
    #     model = xgb_reg()
...
################################# only ocean ##############################3
    ...
    for ndepth in ndepths:
        ...
        for ntree in ntrees:
            ntree_name = str(ntree)+'tree'
            figpath1 = Path(r'G:\DL_transmitance\figures\101L\final\reg_result')/depth_name/ntree_name
            tlog = open(figpath1/'time_log_gbt.txt','w')
    ###################################################################################
            model = gbt_reg(loss,ntree,ndepth)
        ...
        ############################# independent profiles ########################################
            ...
            tlog.close()
    # writer = pd.ExcelWriter(Path(r'G:\DL_transmitance\revised datasets\final')/'bt_results_gbt.xlsx')
