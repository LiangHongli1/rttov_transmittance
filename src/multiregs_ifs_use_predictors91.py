...
def main():
    ...
    figpath = Path(r'G:\DL_transmitance\figures\multi_regressors\layer-by-layer-4feature-selection\ch12\cloud0.3\no outliers\check feature')
    frank = Path(r'G:\DL_transmitance\figures\multi_regressors\layer-by-layer-4feature-selection\ch12\cloud0.3\no outliers')/'feature_ranks.xlsx'

    nlv = 91
    chs = [12]
    fignum = 1
    X,Y,tao_rttov = loadata(datafile)
    feature_rank = np.array(pd.read_excel(frank,header=0).values,dtype=np.int)
    feature_rank = feature_rank[:,1:]
    print(feature_rank[:,0])
    ...
    # rr = np.where(profiles[r,460]==1)[0]
    # emiss = emiss[rr]
    # taosrf = taosrf[rr]
    # bt_true = bt_true[rr]
...
################################################################################################################
        for ii in range(nlv):
            if ii>=1:
                features = np.where(feature_rank[ii-1,:]>0)[0]
                if len(features)<5:
                    pass
                else:
                    trainx = trainx[:,features] # only using top 200 features to check the performance
