...
def gbt_reg(loss):
    reg = GradientBoostingRegressor(n_estimators=90,subsample=0.7,
                                    learning_rate=0.045,
                                    max_depth = 3,
...
def main():
    xfile = Path(r'G:\DL_transmitance\revised datasets\dataset_101L_1500.HDF')
    fp = Path(r'G:\DL_transmitance\profiles\model_level_definition.xlsx')
    fweight = Path(r'G:\DL_transmitance\profiles\normed_weight.xlsx')
    figpath1 = Path(r'G:\DL_transmitance\figures\101L\reg-results\1500profile\+srf2space')
    ...
    with h5py.File(xfile,'r') as f:
        X = f['dependent/X'][:]
        Y = f['dependent/Y'][:]
        tao_rttov = f['dependent/transmission_RTTOV'][:]
        ...
        XX = f['independent/X'][:]
        YY = f['independent/Y'][:]
        tao_rttov2 = f['independent/transmission_RTTOV'][:]
    ...
    # condition = np.logical_or(X[:,100]<280,X[:,308]==1) # kick out land
    condition = X[:,100]<280
    kick_index = np.where(condition)[0]
    X = np.delete(X,kick_index,axis=0)
    X = np.delete(X,[305,306,308,309,310,311],axis=1) # kick out 2m variables and longitude, latitude, srf_type, elevation
    ########## col 303,304,307 are 2m pressure, 2m temperature and skin temperature respectively
    npro = X.shape[0]
    random.seed(2020)
    ...
    XX = np.delete(XX,[305,306,308,309,310,311],axis=1)
    kick_index = np.where(XX[:,100]<280)[0]
    XX = np.delete(XX,kick_index,axis=0)
    YY = np.delete(YY,kick_index,axis=1)
    emiss2 = np.delete(emiss2,kick_index,axis=0)
    tao_rttov2 = np.delete(tao_rttov2,kick_index,axis=0)
    bt_rttov2 = np.delete(bt_rttov2,kick_index,axis=0)
    bt_true2 = np.delete(bt_true2,kick_index,axis=0)
    new_preds2 = np.delete(new_preds2,kick_index,axis=0)
################################# only ocean ##############################3
    ...
    for j,ch in enumerate(chs):
        ...
        for s in seeds:
            ...
            tskin_train,tskin_test = X_train[:,305],X_test[:,305]
            ...
            trans_preds_train = np.ones((X_train.shape[0],nlv+1),dtype=np.float) ## save every laye's predicted transmittance to calculate BT
            trans_preds_test = np.ones((X_test.shape[0],nlv+1),dtype=np.float) ## same as above
            trans_preds_independent = np.ones((XX.shape[0],nlv+1),dtype=np.float)
            ...
            for ii in range(nlv):
                ...
                if ii>0:
                    ...
                    trainx = np.concatenate((X_train[:,[ii,ii+101,ii+202,303,304,305]],newtrain),axis=1)
                    testx = np.concatenate((X_test[:,[ii,ii+101,ii+202,303,304,305]],newtest),axis=1)
                    XXii = np.concatenate((XX[:,[ii,ii+101,ii+202,303,304,305]],new_preds2[:,:,ii-1]),axis=1)
                else:
                    trainx = X_train
                    testx = X_test
                    XXii = XX
                ...
                # XXii = scaler.fit_transform(XXii)
                model = gbt_reg(loss)
            ...
            plot_error_level(bias_levs_train[:-1], rmse_levs_train[:-1],bias_levs_train_rttov[:-1],rmse_levs_train_rttov[:-1],p,title)
            ...
            fignum += 1
            plt.figure(fignum,dpi=300)
            plot_error_level(bias_levs_test[:-1], rmse_levs_test[:-1],bias_levs_test_rttov[:-1],rmse_levs_test_rttov[:-1],p,title)
...
###################### independent profiles validation ###########################3
            ...
            fignum += 1
            plt.figure(fignum,dpi=300)
            plot_error_level(bias_levs[:-1], rmse_levs[:-1],bias_levs_rttov[:-1],rmse_levs_rttov[:-1],p,title)
            ...
            plt.savefig(figpath/figname)
##############################################################################################
            rmse_levs_train2 = np.sum(weights*rmse_levs_train[1:-1])
            rmse_levs_train_rttov2 = np.sum(weights*rmse_levs_train_rttov[1:-1])
            rmse_levs_test2 = np.sum(weights*rmse_levs_test[1:-1])
            rmse_levs_test_rttov2 = np.sum(weights*rmse_levs_test_rttov[1:-1])
...
##############################################################################################################
            ...
            taosrf_train = trans_preds_train[:,-2] #tao_rttov_train[:,-1]

            bt_preds_train,btair_preds_train = bt_GBT(trans_preds_train[:,:-1],tlevel_train,emis_train,tsrf_train,tskin_train,taosrf_train,
...
############################################## Plot good and bad performance profile ###########
            ...
            fignum += 1
            plt.figure(dpi=300)
            plot_trans_level(Y_train[bias_rank[0],:-1],trans_preds_train[bias_rank[0],:-1],tao_rttov_train[bias_rank[0],:-1],p,'predicted transmittane','training good profile')
            figname = channel+'train_trans_good.png'
            plt.savefig(figpath/figname)
            fignum += 1
            plt.figure(dpi=300)
            plot_trans_level(Y_train[bias_rank[-1],:-1],trans_preds_train[bias_rank[-1],:-1],tao_rttov_train[bias_rank[-1],:-1],p,'predicted transmittane','training bad profile')
            figname = channel+'train_trans_bad.png'
            plt.savefig(figpath/figname)

            ################################# test BT########################
            tlevel_test = X_test[:,:nlv]
            taosrf_test = trans_preds_test[:,-2] #tao_rttov_test[:,-1]
    ...
    ##################################### independent profiles #################################
            taosrf = trans_preds_independent[:,-2] #tao_rttov2[:,j,-1]
            bt_preds,btair_preds = bt_GBT(trans_preds_independent[:,:-1],XX[:,:nlv],emiss2[:,j],XX[:,303],XX[:,305],taosrf,
                                                    ch_info[channel]['scale'],ch_info[channel]['offset'],ch_info[channel]['cv'])
...
###############################################################################################################
            ...
            fignum += 1
            plt.figure(dpi=300)
            plot_trans_level(Y_test[bias_rank[0],:-1],trans_preds_test[bias_rank[0],:-1],tao_rttov_test[bias_rank[0],:-1],p,'predicted transmittance','testing good profile')
            figname = channel+'test_trans_good.png'
            plt.savefig(figpath/figname)
            fignum += 1
            plt.figure(dpi=300)
            plot_trans_level(Y_test[bias_rank[-1],:-1],trans_preds_test[bias_rank[-1],:-1],tao_rttov_test[bias_rank[-1],:-1],p,'predicted transmittance','testing bad profile')
            figname = channel+'test_t_bad.png'
            plt.savefig(figpath/figname)
##########################################################################################
