...
def rf_reg(ntree,ndepth):
    model = RandomForestRegressor(n_estimators=ntree,max_depth=ndepth,
...
def main():
    ...
    modelpath = Path(r'G:\DL_transmitance\models\RF')
...
################################# only ocean ##############################3
    ...
    for ndepth in ndepths:
        ...
        for ntree in ntrees:
            ntree_name = str(ntree)+'tree'
            figpath1 = Path(r'G:\DL_transmitance\figures\101L\final\reg_result')/depth_name/ntree_name
            tlog = open(figpath1/'time_log_RF.txt','w')
    ###################################################################################
            model = rf_reg(ntree,ndepth)
            for j,ch in enumerate(chs):
                ...
                for ratio in sample_ratio:
                    ...
                    for ii in range(nlv):
                        ...
                        model_name = 'RF_'+channel+'_level_'+str(ii)+'.m'
        ...
        ##############################################################################################################
                    ...
                    ymin = np.min(ax)
                    ymax = np.max(ax)
                    plot_bias_bt(xland,btair_preds_train[landmask]-btair_train_ytrain[landmask],
                                 btair_train_rttov[landmask]-btair_train_ytrain[landmask],title+' bias of BT_air(land)',method)
                    plt.ylabel('Deviation of BT_air(K)',fontsize=12)
                    plt.ylim([ymin,ymax])
                    plt.subplot(122)
                    plot_bias_bt(xsea,btair_preds_train[seamask]-btair_train_ytrain[seamask],
                                 btair_train_rttov[seamask]-btair_train_ytrain[seamask],title+' bias of BT_air(ocean)',method)
                    plt.yticks([])
                    plt.ylim([ymin,ymax])
        ...
        ############################# independent profiles ########################################
            ...
            tlog.close()
    writer = pd.ExcelWriter(Path(r'G:\DL_transmitance\revised datasets\final')/'bt_results_rf.xlsx')
