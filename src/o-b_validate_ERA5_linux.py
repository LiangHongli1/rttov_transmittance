...
# os.chdir('/home/sky/rttov121/rttov_test')   /mnt/hgfs
...
fprof = prof_path/'ERA5_ensemble_mean_pressure_level_20080714.nc'
fprof2 = prof_path/'make_ERA5_pressure_level_20080714.HDF'
fprof_srf = prof_path/'ERA5_ensemble_mean_single_level_20080714.nc'
fobs = obs_path/dodate/'FY3A_IRASX_GBAL_L1_20080714_0616_017KM_MS.HDF'
fcld = cld_path/dodate/'FY3A_IRASX_GBAL_L1_20080714_0616_017KM_MS_CL.HDF'
...
rs,cs = np.where(mask)
nvaliad = len(rs)
# print('the number of valid obs: %d' % nvaliad)
################################ end #####################################################
...
bt2save = np.zeros((nvaliad,5),dtype=float)
# print(nvaliad)
X2save = np.zeros((nvaliad,306))
m = 0
for k in range(nvaliad):
    ...
    if pres_srf_int<600 or tmp_int[-1]<280 or q_int[-1]<10**-6 or (o3_int<0.1E-8).any() or (q_int<0.1E-10).any():
        ...
        pass
    child_dir_num = 'ERA'+str(m//999+1)
    s = str(m%999+1)
    num = '0'*(3-len(s))+s
    child_dir = parent_dir/child_dir_num/num
...
# figname = 'combine_water_200807140006_interped101.png'
# plt.savefig(figpath/figname)
# plot_profile_var(X2save[:,:101],p101,'Temperature(K)')
# figname = 'combine_tmp_200807140006_interped101.png'
# plt.savefig(figpath/figname)
# plot_profile_var(X2save[:,202:303],p101,'Ozone(kg/kg)')
# figname = 'combine_o3_200807140006_interp101.png'
# plt.savefig(figpath/figname)
