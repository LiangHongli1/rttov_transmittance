...
def gbt_reg(loss):
    reg = GradientBoostingRegressor(n_estimators=280,subsample=0.7,
                                    learning_rate=0.045,
                                    max_depth = 3,
...
def main():
    xfile = Path(r'G:\DL_transmitance\revised datasets\dataset_101L_final.HDF')
    fp = Path(r'G:\DL_transmitance\profiles\model_level_definition.xlsx')
    fweight = Path(r'G:\DL_transmitance\profiles\normed_weight.xlsx')
    figpath1 = Path(r'G:\DL_transmitance\figures\101L\reg-results\1500profile\final')
    ...
    #
    XX = np.delete(XX,[303,305,306,308,309,310,311,312],axis=1)
    rand_num = np.random.randn(XX.shape[0],1)
    XX = np.concatenate((XX,rand_num),axis=1)
...
################################# only ocean ##############################3
    ...
    # seeds = random.sample(range(2019),1)
    loss = 'ls'
    ###################################################################################
