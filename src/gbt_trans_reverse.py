...
def main():
    ...
    figpath1 = Path(r'G:\DL_transmitance\figures\101L\final\reg_result\reverse')
...
################################# only ocean ##############################3
    ...
    # sample_ratio = [0.25,0.5,0.7,0.9]
    sample_ratio = [1]
    height = [[44,89,97],[44,85,97],[44,75,97]]
    ###################################################################################
    for j,ch in enumerate(chs):
        ...
        for ratio in sample_ratio:
            ...
            Xr = X[index,:]
            ############## set the results of RTTOV as true to test the accuracy of IASI convolved transmittance #########
            Yr = tao_rttov[index,:,:]
            tao_rttovr = Y[:,index,:]
            bt_rttovr = bt_true[index,:]
            bt_truer = bt_rttov[index,:]
            emissr = emiss[index,:]
            ...
            _,_,bt_true_train,bt_true_test = train_test_split(Xr,bt_truer[:,j],test_size=0.2,random_state=seed)
            ...
            plot_bias_ratio_lev(ratio_gbt,ratio_rttov,p,title)
            ...
            plot_std_lev(ratio_gbt,ratio_rttov,p,title)
...
##############################################################################################################
            ...
            ymin = np.min(ax)
            ymax = np.max(ax)
            plot_bias_bt(xland,btair_preds_train[landmask]-btair_train_ytrain[landmask],btair_train_rttov[landmask]-btair_train_ytrain[landmask],title+' bias of BT_air(land)')
            plt.ylabel('Deviation of BT_air(K)',fontsize=12)
            plt.ylim([ymin,ymax])
            plt.subplot(122)
            plot_bias_bt(xsea,btair_preds_train[seamask]-btair_train_ytrain[seamask],btair_train_rttov[seamask]-btair_train_ytrain[seamask],title+' bias of BT_air(ocean)')
            plt.yticks([])
            plt.ylim([ymin,ymax])
            ...
            plt.figure(fignum,dpi=300)
            plt.subplot(121)
            plot_bias_bt(xland,bias_pred_train[landmask],bias_rtt_train[landmask],title+' land')
            plt.ylabel('Deviation of BT(K)',fontsize=12)
            plt.ylim([ymin,ymax])
            plt.subplot(122)
            plot_bias_bt(xsea,bias_pred_train[seamask],bias_rtt_train[seamask],title+' ocean')
            plt.ylim([ymin,ymax])
            plt.yticks([])
...
############################################## Plot good and bad performance profile ###########
            ...
            plt.figure(fignum,dpi=300)
            plt.subplot(121)
            plot_bias_bt(xland,btair_preds_test[landmask]-btair_test_ytest[landmask],btair_test_rttov[landmask]-btair_test_ytest[landmask],title+' bias of BT_air(land)')
            plt.ylabel('Deviation of BT_air(K)',fontsize=12)
            plt.ylim([ymin,ymax])
            plt.subplot(122)
            plot_bias_bt(xsea,btair_preds_test[seamask]-btair_test_ytest[seamask],btair_test_rttov[seamask]-btair_test_ytest[seamask],title+' bias of BT_air(ocean)')
            plt.yticks([])
            plt.ylim([ymin,ymax])
            ...
            plt.figure(fignum,dpi=300)
            plt.subplot(121)
            plot_bias_bt(xland,bias_pred_test[landmask],bias_rtt_test[landmask],title+' land')
            plt.ylabel('Deviation of BT(K)',fontsize=12)
            plt.ylim([ymin,ymax])
            plt.subplot(122)
            plot_bias_bt(xsea,bias_pred_test[seamask],bias_rtt_test[seamask],title+' ocean')
            plt.yticks([])
            plt.ylim([ymin,ymax])
